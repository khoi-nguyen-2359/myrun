...
class OtpDialogFragment : DialogFragment() {
	...
	private val phone by lazy { arguments?.getString(ARG_PHONE) ?: throw Exception("Phone is missing") }
	...
	private fun verifyPhoneNumber() {
		...
		PhoneAuthProvider.getInstance().verifyPhoneNumber(
			phone, OTP_VERIFY_TIMEOUT, TimeUnit.SECONDS, requireActivity(), object : PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
			...
			viewBinding.resendButton.post {
				if (!isDetached) {
					...
					if (count == 0L) {
						...
						viewBinding.resendButton.text = getString(R.string.otp_resend_template, count)
